///scr_checkForJumpConnections(node id, direction in terms of blockSize);
...
if (place_meeting(xx, yy, oSolid)){
    ...
    velocityY = -curActor.jspd/ blockSize;
...
for(var p=0; p < array_length_2d(dataBinJ,1); p++){
    ...
    */
    ...
    /*
    with (curActor){
        if (instance_place(xx + (width/2*dir), yy, oSolid))
            return 0;
    }
    */
    
    // make it so that jumping and hitting your head reduces horizontal movement by 1
    // or something, jumping is funny
